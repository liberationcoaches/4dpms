...
export async function createManager(
  ...
  try {
    ...
    const manager = new User({
      ...
      reportsTo: boss._id, // Manager reports to the boss who created them
...
export async function getManagers(
  ...
  try {
    ...
    // Get all direct reports who are managers under this boss
...
export async function getBossAnalytics(
  ...
  try {
    ...
    // Get all managers (direct reports of the boss)
    const managers = await User.find({
      reportsTo: boss._id,
      role: 'manager',
    });

    // Get all employees under these managers (direct reports of each manager)
...
export async function addManagerFunctionalKRA(
  ...
  try {
    ...
    // Validate manager reports to this boss
    const manager = await User.findById(managerId);
    if (!manager || manager.role !== 'manager' || manager.reportsTo?.toString() !== boss._id.toString()) {
      res.status(403).json({
        status: 'error',
